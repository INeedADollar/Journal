...
class JournalWindow(QWidget):
    ...
    def __set_pages(self):
        self.ui.leftPage(self.journal_pages[self.left_page_number]["content"])
        self.ui.leftPageNumber.setText(f"{self.left_page_number + 1}")
        
        self.ui.rightPage(self.journal_pages[self.left_page_number + 1]["content"])
    ...
    def __handle_retrieve_journal(self, message):
        ...
        try:
            with zipfile.ZipFile(io.BytesIO(message["additional_content"])) as journal_zip:
